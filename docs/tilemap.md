...
`tiles`配列は1次元配列で、以下の式でインデックスを計算します：
...
`tiles`配列は、マップの高さごとに改行されています。これにより、視覚的にマップの構造を把握しやすくなっています。
...
1. タイルマップを読み込む
2. Spawns情報を位置でマップ化（高速検索のため）
3. タイルマップを走査してエンティティを生成
4. 各タイルタイプに応じて適切なエンティティを作成
